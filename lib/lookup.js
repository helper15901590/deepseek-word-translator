...
(function (root, factory) {
  ...
  function buildMessages(word, context) {
    ...
    const morphologyHint =
      '"morphology" 请给出该单词的词根词缀拆分，用加号连接各部分并标注类型与含义，例如 “im-（否定前缀）+ mort（词根：死亡）+ -al（形容词后缀）”；无法拆分的基础词或外来词，说明其来源或标注“不可拆分”。';
    const userContent = hasContext
      ? '给出单词 "' + word + '" 的音标和 1~3 条最常用中文释义。该词出现的语境："' +
        context.trim() +
        '"。"contextMeaning" 请给出结合该语境、该词在此处的准确中文含义（一句话）。' +
        morphologyHint +
        '严格按此格式输出：' +
        '{"phonetic":"<IPA音标>","definitions":[{"pos":"<词性缩写，如 n./v./adj./adv.>","meaning":"<中文释义>"}],"contextMeaning":"<结合语境的一句话中文含义>","morphology":"<词根词缀拆分及各部分含义>"}'
      : '给出单词 "' + word + '" 的音标和 1~3 条最常用中文释义。' +
        morphologyHint +
        '严格按此格式输出：' +
        '{"phonetic":"<IPA音标>","definitions":[{"pos":"<词性缩写，如 n./v./adj./adv.>","meaning":"<中文释义>"}],"morphology":"<词根词缀拆分及各部分含义>"}';
    return [
  ...
  // 返回 { ok:true, data:{word,phonetic,definitions,contextMeaning,morphology} } 或 { ok:false, error:"PARSE" }
  function parseLookupResponse(text, word) {
    ...
    const contextMeaning =
      typeof data.contextMeaning === "string" && data.contextMeaning.trim() !== ""
        ...
        : null;
    const morphology =
      typeof data.morphology === "string" && data.morphology.trim() !== ""
        ? data.morphology.trim()
        : null;
    return {
      ok: true,
      data: {
